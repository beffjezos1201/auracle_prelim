...
from net.utils import apply_preprocess_acc
...
class SequentialGenerator(keras.utils.Sequence):
    ...
    def __init__(self, config, recs, segments, batch_size=32, shuffle=False, verbose=True):
        ...
        rec_data = Data.loadData(config.data_path, recs[prev_rec], modalities=['mov'])
        if rec_data is not None:
            rec_data = apply_preprocess_acc(config, rec_data)

        for s in segments:
            curr_rec = int(s[0])
            
            if curr_rec != prev_rec:
                rec_data = Data.loadData(config.data_path, recs[curr_rec], modalities=['mov'])
                if rec_data is not None:
                    rec_data = apply_preprocess_acc(config, rec_data)
                prev_rec = curr_rec
            ...
            if (rec_data is None) or (stop_seg > rec_data.shape[1]):
                # zero-fill all channels when data missing/too short
                for ch in range(self.config.CH):
                    self.data_segs[count, :, ch] = np.zeros(config.fs*config.frame)
            else:
                # fill as many channels as available (up to config.CH)
                for ch in range(self.config.CH):
                    if ch < rec_data.shape[0]:
                        self.data_segs[count, :, ch] = rec_data[ch][start_seg:stop_seg]
                    else:
                        self.data_segs[count, :, ch] =  np.zeros(config.fs*config.frame)
...
class SegmentedGenerator(keras.utils.Sequence):
    ...
    def __init__(self, config, recs, segments, batch_size=32, shuffle=True, verbose=True):
        ...
        pbar = tqdm(total = len(segs_to_load)+1, disable = not self.verbose)
        count = 0
        ...
        while segs_to_load:
            ...
            rec_data = Data.loadData(config.data_path, recs[curr_rec], modalities=['mov'])
            if rec_data is None:
                # Fill zeros for all segments of this recording and continue
                for r in comm_recs:
